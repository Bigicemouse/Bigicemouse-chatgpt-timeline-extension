...
This v2.00 update improves the ChatGPT Timeline Navigator hover preview for real long conversations. It focuses on three practical issues: toolbar alignment, a calmer neutral color palette, and the 19+ conversation freeze where the preview list could become hard to scroll or click.
...
- The previous red/blue accents are replaced with a quieter green-gray palette.
...
- Active rows and markers use neutral green-gray accents instead of red or blue.
...
- `src/ui/styles.js` defines the two-row toolbar layout, tighter controls, and neutral green-gray accent colors.
...
本次 v2.00 更新主要优化 ChatGPT Timeline Navigator 在真实长对话中的悬浮预览体验，重点解决三个问题：顶部工具栏对齐、界面强调色更克制，以及 19 条以上对话时预览列表滚动、点击变卡的问题。
...
- 鼠标悬浮 rail 后打开对话目录卡片。
- 桌面端工具栏稳定两行对齐。
- active 行和 marker 使用中性绿色/灰绿色，不再使用偏红或偏蓝强调色。
...
- `src/ui/styles.js` 定义两行 toolbar 布局、更统一的控件样式，以及绿色/灰绿色强调色。
